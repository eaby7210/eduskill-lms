...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        try:
            logger.info(f"Notification WebSocket connection attempt.\
                URL Route: {
            ...
            await self.accept()
            logger.info(f"Notification WebSocket\
    ...
    async def sendnotification(self, event):
        ...
        try:
            ...
            if target_user and str(self.scope['user'].id) == target_user:
                should_send = True
                logger.debug(
                    f"Matched user-specific notification for user {
                        target_user
                    }")
            ...
            if should_send:
                ...
                logger.info(f"Notification not sent. Target role: {
                            target_role}, Target user: {target_user}")
